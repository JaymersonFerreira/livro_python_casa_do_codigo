...
def read_metadata(filename):
    ...
    for column in read_lines(filename):
        if column:
            metadata.append(tuple(column.split('\n')[:3]))
    return metadata


def prompt():
    print('\nO que deseja ver: ')
    print("(l) Lista entidades")
    print("(d) Exibir atributos de uma entidade")
    print("(r) Exibir referência de uma entidade")
        ...
        for col in val:
            if col[0] in keys:
                if not col[0] == meta[key][0][0]:
                    relationships[keys] = keys[col][0]
    
    opcao = prompt()
    while opcao != 's':
        if opcao == 'l':
            for entity_name in meta[key]:
                print(entity_name)
        elif opcao == 'd':
            entity_name = input('Nome da endidade: ')
